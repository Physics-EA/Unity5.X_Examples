...
public class PlayerAction : MonoBehaviour
{
    ...
    private const float m_ValutMatchTargetStart = 0.4f;
    private const float m_ValutMatchTargetStop = 0.51f;
    private const float m_SlideMatchTargetStart = 0.11f;
    private const float m_SlideMatchTargetStop = 0.4f;
    ...
    void ProcessVault()
    {
        ...
        if (m_Animator.GetCurrentAnimatorStateInfo(0).IsName("Locomotion.Run"))
        {
            if (Physics.Raycast(transform.position + new Vector3(0, 0.3f, 0), dir, out hit, 10))
            {
                ...
                {
                    m_Target = hit.point;
                    m_Target.y = hit.collider.bounds.center.y + 0.5f * GetComponent<Collider>().bounds.extents.y + 0.75f;
                    vault = (hit.distance < 4.5f && hit.distance > 4);
                }
            }
        }
    ...
    void ProcessMatchTarget()
    {
        ...
        if (info.IsName("Base Layer.Vault"))
        {
            if (MatchTarget)
            {
                m_Animator.MatchTarget(m_Target, q, AvatarTarget.LeftHand, new MatchTargetWeightMask(Vector3.one, 0), m_ValutMatchTargetStart, m_ValutMatchTargetStop);
            }
        }
